...
struct Graph {
    ...
    bool read_graph(const char *file_name) 
	{
		...
		for (unsigned i = 0; i < no_edges; i+=2) 
		{
			...
			edges[i+1].nodes[0] = edges[i].nodes[1];
			edges[i+1].nodes[1] = edges[i].nodes[0];
			edges[i+1].type = edges[i].type;
			edges[i+1].distance = edges[i].distance;
			edges[i+1].capacity = edges[i].capacity;
		}
		file.close();
    }
